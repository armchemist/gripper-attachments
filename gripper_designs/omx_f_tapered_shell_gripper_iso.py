...
설계 개요 (이등변 삼각형 측면 프로파일 버전):
  - 하단 (Y=cut_y) 은 원본 STL 의 모터 마운트/하우징을 그대로 사용.
  - 베이스 단면 X·Z 는 하우징 envelope 와 매칭 (Z=-36..4).
  - 팁 단면은 베이스 중심 Z (= (base_z_bottom + base_z_top)/2) 기준 위/아래 대칭으로 좁아짐.
  - 결과: 옆에서 봤을 때(YZ) 측면 프로파일이 이등변 삼각형 — 윗변·바닥 모두 안쪽으로 슬로프.
  - 안쪽(-X) 면은 길이방향으로 평면 유지 (그립 안정성).
  - 외벽만 남기고 안을 비운 쉘 (boolean cut, loft 인셋).
  - 내부에 일정 간격 가로 지지대(ribs).
  - +X 면 ridge — 세로 길쭉한 삼각 돌기.
  - -X 면 lightening 컷아웃 — rib 사이 트러스.
"""
...
# tip_z_bottom = -16 - 5 = -21, tip_z_top = -16 + 5 = -11
tip_z_height    = 10.0                               # 팁 단면 Z 높이 (mm) — 이등변 대칭 폭
_blade_z_center = (base_z_bottom + base_z_top) / 2.0 # 측면 이등변의 대칭축 Z (= -16)
tip_z_bottom    = _blade_z_center - tip_z_height/2   # 팁 바닥 Z — 중심에서 위로 올라옴 (슬로프 down→up)
tip_z_top       = _blade_z_center + tip_z_height/2   # 팁 윗변 Z — 중심에서 아래로 내려옴 (슬로프 down)
...
# ── 출력 ─────────────────────────────────────────────────
OUT_BLADE_STL = "omx_f_tapered_shell_iso_blade_only.stl"  # 블레이드만 (디버그)
OUT_FINAL_STL = "omx_f_tapered_shell_iso_gripper_L.stl"   # 최종 union
