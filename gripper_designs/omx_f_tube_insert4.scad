...
// Full-finger phantom (pivot + housing + entire blade x=-10..65).
// Hull of slabs over the whole link6 finger envelope from STL data.
// Cyan translucent — for visual alignment of insert vs full gripper.
full_finger_pts = [
    // [ x, half_Y, half_Z ]  — housing/pivot uses bbox half-extents
    [-10, 10.0, 21.3 ],   // pivot shaft / housing back
    [  0, 10.0, 21.3 ],   // housing front
    [ 22, 10.0, 21.3 ],   // housing/blade boundary (housing dominates here)
    [ 22,  5.7, 19.4 ],   // blade base
    [ 28,  4.2, 17.7 ],
    [ 36,  3.1, 15.5 ],
    [ 46,  2.8, 12.7 ],
    [ 55,  3.5, 10.3 ],
...
module finger_zone_hull(i0, i1) {
    hull()
        for (i = [i0:i1]) {
            p = full_finger_pts[i];
            translate([p[0], 0, 0])
                cube([0.01, p[1]*2, p[2]*2], center=true);
        }
}

module blade_phantom() {
    color("cyan", 0.25) {
        // Housing block (x=-10..22): straight box, no taper
        finger_zone_hull(0, 2);
        // Blade taper (x=22..65)
        finger_zone_hull(3, len(full_finger_pts)-1);
    }
}
