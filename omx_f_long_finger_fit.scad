...
// ── PHANTOM STL 경로 (socket_cavity 에서도 사용) ─────────────
...
// ★ blade 단면 곡률 반영: 직사각형 대신 모서리가 둥근 단면 사용
// blade_pts 좌표계 그대로 사용하므로 구멍이 정확히 생성됨.
// r_frac 로 라운딩 비율 조정 (0=직사각, 0.5=타원 근사).
// 단면 Y 반폭(hy) 기준으로 r = hy * r_frac 계산.
module socket_cavity() {
    ...
    hull()
        for (i = [0:N_HULL]) {
            ...
            hy = bhy(x) + clearance;
            hz = bhz(x) + clearance;
            r  = min(hy * r_frac, hz * r_frac, 3.0);
            // rotate([0,90,0]) 후: 2D X→3D Z, 2D Y→3D Y, 두께(h)→3D X
            translate([x, 0, 0])
            rotate([0, 90, 0])
...
//  소켓 피팅 (STL 곡률 반영 버전)
//  ───────────────────────────────
//  clearance 는 진입부(x=22) 기준 비례 스케일 적용.
//  x=65 쪽 유효 클리어런스는 clearance×0.5 수준.
//  헐겁다       → clearance 줄임 (0.20)
//  빡빡함       → clearance 늘림 (0.40~0.50)
